...
**Day2 交付物（两档）**
- **最小成功**：Scene1 稳定可演示（抓→看→扔→TTS）+ `/status` 可回传
- **2日内最大成功**：Scene1 稳定 + Scene2/3/4 至少 2 个可触发（点三点/点头/摇头）+ 台词风格（polite/meme）+ Demo 视频 + README + Slide
...
## 5. 风险与保底策略（必须写死）
- **抓取不稳（本项目最大风险）**：不允许推/拨保底，因此策略是：
  - 先把“抓一张牌”做成单独验收项（反复调到稳定）
  - 优先把场景简化：固定抓取位置/固定牌姿态/固定光照
  - 必要时换夹爪垫片/增加摩擦（TPU/胶带/防滑垫）
- 视觉识别不稳：允许降级为“手动点击画面取点/固定目标区坐标”（但抓取必须成功）
