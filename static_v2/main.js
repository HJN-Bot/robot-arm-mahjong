...
let autoLoop       = false;   // Watch Mode: true = 等待 trigger_pending 自动执行
let autoLoopRunning = false;  // 防止重入
...
// cx 
// ===== Scene A video control =====
function playSceneAVideo() {
  const v = $("avatar-video-scene-a");
  if (!v) return;
   hideAllAvatarVideos(); 

  // show video, hide existing avatar layers
  v.style.display = "block";

  const img = $("avatar-anim");
  if (img) img.style.display = "none";

  const emoji = $("avatar-emoji-big");
  if (emoji) emoji.style.display = "none";

  // start from beginning & play
  try { v.currentTime = 0; } catch (_) {}
  const p = v.play();
  if (p && typeof p.catch === "function") {
    p.catch(err => appendLog("[VIDEO] SceneA 播放失败: " + err.message));
  }
}

function stopSceneAVideo() {
  const v = $("avatar-video-scene-a");
  if (!v) return;
  ...
  // restore avatar layers (let setAvatar decide final visuals)
  const img = $("avatar-anim");
...
// cx 
// ===== Scene B video control =====
function playSceneBVideo() {
  const v = $("avatar-video-scene-b");
  if (!v) return;
   hideAllAvatarVideos(); 
  ...
  const p = v.play();
  if (p && typeof p.catch === "function") {
    p.catch(err => appendLog("[VIDEO] SceneB 播放失败: " + err.message));
  }
}

function stopSceneBVideo() {
  const v = $("avatar-video-scene-b");
  if (!v) return;
...
function hideAllAvatarVideos() {
  const ids = ["avatar-video-scene-a", "avatar-video-scene-b"];
  for (const id of ids) {
    const v = $(id);
    if (!v) continue;
    try { v.pause(); } catch (_) {}
...
async function getStatus() {
  try {
    ...
    // Remote activate: OpenClaw → 自动开摄像头 + 启动 Watch Mode
    if (j.activate_pending) {
      appendLog("[REMOTE] activate → 开启摄像头 + Watch Mode");
      if (!cameraStream) startCamera();
      autoLoop = true;
    }

    // Remote trigger: OpenClaw → 执行一次 autoLoopTick
    if (j.trigger_pending && autoLoop && !autoLoopRunning) {
      appendLog("[REMOTE] trigger → autoLoopTick");
      autoLoopTick();
    }
...
async function runScene(scene) {
  ...
  // cx
  if (scene === "A") playSceneAVideo();
  //cxstop

   // cx
  if (scene === "B") playSceneBVideo();
  //cxstop
...
// ===== AutoLoop Tick (Discord → trigger → 自动流程) =====
async function autoLoopTick() {
  ...
  appendLog("[AUTO] ▶ 开始自动流程");
  setAvatar("acting", "来！开牌...");

  // Step 1: 通知后端启动机械臂 pick + present（当前 mock 模式直接返回）
  try {
    const r1 = await fetch(`/arm/start_scene?style=${style}&safe=${safe}`, { method: "POST" });
    const d1  = await r1.json().catch(() => ({}));
    if (d1.ok === false) {
      appendLog("[AUTO] arm/start_scene 失败: " + (d1.error || "unknown"));
      setAvatar("error", "启动失败");
      setTimeout(() => setAvatar("idle"), 3000);
      autoLoopRunning = false;
      return;
    }
  } catch (e) {
    appendLog("[AUTO] arm/start_scene 异常: " + e.message);
    setAvatar("error", "网络错误");
    setTimeout(() => setAvatar("idle"), 3000);
    autoLoopRunning = false;
    return;
  ...
  // Step 2: 浏览器截帧（480×360，质量0.75）→ KIMI Vision 识别
  setAvatar("thinking", "正在识别牌面...");
  const video  = $("camera-feed");
  const canvas = $("camera-canvas");
  let bestRec  = null;

  await new Promise(r => setTimeout(r, 300)); // 等臂稳定
  if (!cameraStream || !video || video.readyState < 2) {
    appendLog("[AUTO] 摄像头未就绪，请先开启摄像头");
    setAvatar("error", "摄像头未就绪");
    setTimeout(() => setAvatar("idle"), 3000);
    autoLoopRunning = false;
    return;
  }
  // 缩图到 480×360（数据量从 ~250KB → ~25KB，KIMI 推理快 ~2-3x）
  canvas.width  = 480;
  canvas.height = 360;
  ...
  try {
    const r = await post("/capture_frame", { image: base64 });
    const conf = r.recognized?.confidence || 0;
    appendLog(`[AUTO] KIMI识别: ${r.recognized?.label || "??"} (${Math.round(conf * 100)}%)`);
    if (r.recognized) bestRec = r.recognized;
  } catch (e) {
    appendLog("[AUTO] 识别请求失败: " + e.message);
  }

  if (!bestRec) {
    appendLog("[AUTO] ✗ 识别失败，无有效帧");
    setAvatar("error", "识别失败");
    setTimeout(() => setAvatar("idle"), 3000);
    autoLoopRunning = false;
    return;
  ...
  renderRecognized(bestRec);
  appendLog(`[AUTO] ✓ 最终识别: ${bestRec.label} (${Math.round(bestRec.confidence * 100)}%)`);

  // Step 3: 根据识别结果自动路由 Scene A / B，执行机械臂动作
  const scene = bestRec.label === "white_dragon" ? "A" : "B";
  setAvatar("acting", scene === "A" ? "白板，扔出！" : "一筒，留下！");
  if (scene === "A") playSceneAVideo();
  else               playSceneBVideo();

  const t0 = Date.now();
  try {
    const r3 = await post("/execute_scene", {
      scene,
      style,
      safe,
      recognized_label: bestRec.label,
      recognized_conf:  bestRec.confidence,
    });
    const ms = Date.now() - t0;

    history.push({ scene, ok: r3.ok, ms, label: bestRec.label, conf: bestRec.confidence, style });
    renderStats();
    renderHistory();

    setAvatar(r3.ok ? "done" : "error",
      r3.ok
        ? (scene === "A" ? "验牌完毕，扔出！" : "验牌完毕，留下！")
        : "执行失败");
    setTimeout(() => setAvatar("idle"), 3000);
  } catch (e) {
    appendLog("[AUTO] execute_scene 异常: " + e.message);
